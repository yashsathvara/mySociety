...
export default function OwnerForm () {
  ...
  const handleUpdateOwner = async () => {
    ...
    try {
      setIsLoading(true)
      const response = await UpdateOwner(resident._id, formData)
      toast.success(response.data.message)
      navigate('/residentmanagement')
    } catch (error) {
      console.log(error)
      ...
      <div className="bg-white rounded-lg p-6 shadow-md ">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
          ...
          <div className='md:col-span-10'>
                  ...
                  <div className='absolute mt-1 bg-white p-2 w-full z-10 shadow-[0px_0px_40px_0px_#00000014] ps-[20px] py-[20px] rounded-[10px]'>
                    ...
                    >
                      <input
                        ...
                        onChange={handleInputChange}
                        className='mr-2 custom-radio mb-[10px]'
                      />
                      Male
                    </label>
                    <label
            ...
            <div className='grid grid-cols-4 gap-4 mt-8 max-lg:grid-cols-2 max-sm:grid-cols-1 max-xl:grid-cols-2 max-2xl:grid-cols-2'>
              {/* Aadhar Front */}
              <div>
            ...
            {resident?.Member_Counting ? (
              <>
                {formData.Member_Counting.map((member, index) => (
                  <div
                    key={index}
                    className='grid grid-cols-6 gap-4 mt-4 items-start max-lg:grid-cols-2 max-xl:grid-cols-1 max-sm:grid-cols-1'
                  >
                    <div>
              ...
              <>
                ...
                {[...Array(memberCount)].map((_, index) => (
                  <div
                    key={index}
                    className='grid grid-cols-6 gap-4 mt-4 items-start max-lg:grid-cols-3 max-xl:grid-cols-2 max-sm:grid-cols-1'
                  >
                    <div>
                      <label className='block text-sm text-black-500 font-lighter mb-1'>
                        Full Name*
